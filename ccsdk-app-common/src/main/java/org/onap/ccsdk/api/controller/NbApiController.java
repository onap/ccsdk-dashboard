...
public class NbApiController extends ApiBaseController {
    ...
    private boolean isAuthorized(HttpServletRequest request, String component) {
        ...
        String reqRole = "ECOMPC_" + component.toUpperCase() + "_WRITE";
        Predicate<String> compWritePred = p -> p.contains("ECOMPC_WRITE") || p.contains(reqRole);
        Predicate<String> dcaeWritePred = p -> p.contains("ECOMPC_DCAE_WRITE");
        Predicate<String> dc2aWritePred = p -> p.contains("ECOMPC_D2A_WRITE");
        ...
        if (userRoleSet != null) {
            if (userRoleSet.stream().noneMatch(adminPred)) {
                if (component.equalsIgnoreCase("dcae")
                    && !(userRoleSet.stream().anyMatch(dcaeWritePred))) {
                    return false;
                }
                if (!component.equalsIgnoreCase("dcae")
                    && !(userRoleSet.stream().anyMatch(compWritePred))) {
                    return false;
                }
                if (component.equalsIgnoreCase("d2a")
                    && !(userRoleSet.stream().anyMatch(dc2aWritePred))) {
                    return false;
                }
            }
        }
