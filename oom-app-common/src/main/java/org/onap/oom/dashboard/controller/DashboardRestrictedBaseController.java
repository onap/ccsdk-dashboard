...
import org.onap.oom.dashboard.service.ControllerEndpointService;
import org.onap.oom.dashboard.util.DashboardProperties;
...
public class DashboardRestrictedBaseController extends RestrictedBaseController {
     ...
     */
    private static EELFLoggerDelegate log = EELFLoggerDelegate.getLogger(DashboardRestrictedBaseController.class);
    ...
    protected ControllerEndpointCredentials[] getControllerEndpoints() {
        ...
        for (int i = 0; i < controllerKeys.length; ++i) {
            ...
            log.debug(EELFLoggerDelegate.debugLogger, "getConfiguredControllers: key {} yields url {}", key, url);
     ...
     */
    protected IControllerRestClient getControllerRestClient(HttpServletRequest request)
        throws DashboardControllerException {
        ...
        if (mock) {
            ...
            try {
                ...
                final String clearText =
                    details.getEncryptedPassword() ? details.decryptPassword() : details.getPassword();
                result = new ControllerRestClientImpl(details.getUrl(), details.getUsername(), clearText);
            } catch (Exception ex) {
                log.error("getControllerRestClient failed", ex);
