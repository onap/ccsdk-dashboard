...
public class ECDSingleSignOnController extends UnRestrictedBaseController {
    ...
    private EELFLoggerDelegate log = EELFLoggerDelegate.getLogger(ECDSingleSignOnController.class);
        ...
        if (isLoginCookieExist(request) && redirectToPortal == null) {
            ...
            if (session == null || user == null) {
                ...
                try {
                    ...
                    log.error("singleSignOnLogin failed", ex);
                ...
                if (commandBean.getUser() == null) {
                    ...
                    final String redirectUrl = PortalApiProperties.getProperty(PortalApiConstants.ECOMP_REDIRECT_URL)
                        + "?noUserError=Yes";
                    log.debug(EELFLoggerDelegate.debugLogger, "singleSignOnLogin: user is null, redirect URL is {}",
                        redirectUrl);
                ...
                log.info(EELFLoggerDelegate.debugLogger, "singleSignOnLogin: redirecting to the forwardURL {}",
                    forwardURL);
                return new ModelAndView(REDIRECT + forwardURL);
            }
        } else {
            ...
            if (isPortalAvailable()) {
                ...
                if (SystemProperties.containsProperty(SystemProperties.APP_BASE_URL)) {
                    ...
                    log.debug(EELFLoggerDelegate.debugLogger, "singleSignOnLogin: computed redirectURL {}",
                        returnToAppUrl);
                }
                ...
                log.debug(EELFLoggerDelegate.debugLogger, "singleSignOnLogin: portal-bound redirect URL is {}",
                    redirectUrl);
                return new ModelAndView(REDIRECT + redirectUrl);
            } // portal is available
    ...
    private boolean isPortalAvailable() {
        ...
        try {
            ...
            log.trace("isPortalAvailable got response {}", s);
        } catch (RestClientException ex) {
            log.debug("isPortalAvailable failed", ex);
            avail = false;
        }
        return avail;
    }
